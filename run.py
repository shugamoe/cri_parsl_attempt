...
# Base file paths to work with
dose_paths = glob.glob("../input/geno_train/*train.gz")

dose_paths = [path for path in dose_paths if "chr23" not in path] # Remove chr23
# dose_paths = [path for path in dose_paths if "chr22.p1" in path] # Run a single file, works, caches result 07ad077935bab8ede17ab280b1ebb596
# dose_paths = [path for path in dose_paths if "chr22.p2" in path] # Run a single file, works, caches result 47a423c6e71c58eef057a2d95cb7522a


# The below tries to run 2 files, it can find the 2 cached results above,
# but if run without cached results available, it submits the 2 tasks with the exact
# same memoization hash, but each task never completes for some reason.
# dose_paths = [path for path in dose_paths if ("chr22.p1" in path) or # ("chr22.p2" in path)] 
# Hashes when finding cached results (would have to match to use the cached results)
...
# dose_paths = [path for path in dose_paths if "chr22" in path] # Select Chr22 only (not run yet)

# Get input File objects from paths above
...
# Get output File objects from paths above
...
print(dose_files)
print(gwas_files)
print(filtered_dose_files)
for i in filtered_dose_res:
    print(i.result())
# ### Step 1 ###
